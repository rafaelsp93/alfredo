...
func run(opts runOptions) error {
	...
	changedLines, changedFiles, err := changedGoLines(opts.RepoRoot, mergeBase, opts.Mode == "local")
	if err != nil {
...
func runGovulncheck(opts runOptions, mergeBase string, changed map[string]map[int]bool) stepResult {
	...
	hasVulnerabilities := strings.Contains(res.Output, "Vulnerability #")
	if hasVulnerabilities {
		...
		if changedDependency || changedTrace {
			...
			step.Summary = "govulncheck found vulnerabilities"
		}
		return step
	}
	if res.Err != nil {
...
func runGosec(opts runOptions, changed map[string]map[int]bool, failSeverities []string, report *reportData) []stepResult {
	...
	findings, err := parseGosec(jsonPath, failSeverities)
	if err != nil {
		step.Status = statusFail
		step.Summary = err.Error()
		return []stepResult{step}
	}
	report.NewSASTFindings = filterChangedFindings(findings, changed)
	if len(findings) > 0 {
		step.Status = statusFail
		if len(report.NewSASTFindings) > 0 {
			step.Summary = fmt.Sprintf("%d high-or-worse SAST finding(s), %d on changed lines", len(findings), len(report.NewSASTFindings))
		} else {
			step.Summary = fmt.Sprintf("%d high-or-worse SAST finding(s)", len(findings))
		}
	} else {
		step.Summary = "no high-or-worse SAST findings"
	}
	return []stepResult{step}
}
...
func changedGoLines(repoRoot, base string, cached bool) (map[string]map[int]bool, []string, error) {
	args := []string{"diff", "--unified=0", "--no-ext-diff"}
	if cached {
		args = append(args, "--cached")
	}
	args = append(args, base, "--", "*.go")
...
func parseGosec(path string, failSeverities []string) ([]finding, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		if errors.Is(err, os.ErrNotExist) {
			return nil, nil
...
func filterChangedFindings(findings []finding, changed map[string]map[int]bool) []finding {
	filtered := make([]finding, 0, len(findings))
	for _, finding := range findings {
		if isChangedLine(changed, finding.File, finding.Line) {
